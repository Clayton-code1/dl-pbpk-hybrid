...
Uses RDKit for parsing and featurisation. Returns plain PyTorch tensors so
no PyTorch Geometric / DGL dependency is needed.
"""
...
_NUM_HYBRID = len(_HYBRIDISATION_MAP) + 1  # +1 for "Other"

NODE_FEAT_DIM = _NUM_ATOM_TYPES + (_MAX_DEGREE + 1) + 1 + 1 + _NUM_HYBRID + (_MAX_NUM_H + 1)
# 10 + 6 + 1 + 1 + 4 + 5 = 27

# ── bond-level one-hot vocabularies ──────────────────────────────────────
...
EDGE_FEAT_DIM = _NUM_BOND_TYPES + 1 + 1  # 4 + conjugated + in_ring = 6
...
def _atom_features(atom: rdchem.Atom) -> list[float]:
    ...
    atom_idx = _ATOM_MAP.get(symbol, len(_ATOM_TYPES))  # "Other"
...
def smiles_to_graph(smiles: str) -> dict[str, Tensor]:
    ...
    -------
    dict with keys:
        ``x``          – FloatTensor [num_atoms, NODE_FEAT_DIM]
        ``edge_index`` – LongTensor  [2, num_edges]  (undirected: both directions)
        ``edge_attr``  – FloatTensor [num_edges, EDGE_FEAT_DIM]

    Raises
    ------
    InvalidSMILESError
        If *smiles* cannot be parsed or produces an empty molecule.
    """
