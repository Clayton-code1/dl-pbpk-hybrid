...
GET  /explainability/graph/{drug}  Pre-computed atom-level attribution from graph_shap_lite
...
from app.schemas import (
    GraphAtomAttribution,
    GraphAttributionResponse,
    HealthResponse,
    ModelStateResponse,
    ModelUpdateRequest,
    ModelUpdateResponse,
...
_GRAPH_ATTR_CSV = _PROJECT_ROOT / "experiments" / "results" / "phase3b_graph_explainability.csv"

_GRAPH_ATTR_DRUGS = frozenset({
    "acetaminophen", "caffeine", "digoxin", "midazolam", "theophylline", "warfarin"
})
...
@app.get(
    ...
    response_model=GraphAttributionResponse,
    tags=["explainability"],
)
async def graph_attribution(drug: str):
    """Return pre-computed atom-level attribution scores for *drug*.

    Results are read from the static CSV generated by
    experiments/explainability/graph_shap_lite.py.  No model inference is run.
    """
    slug = drug.strip().lower()
    if slug not in _GRAPH_ATTR_DRUGS:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Drug '{drug}' not found in pre-computed attribution results. "
                f"Available drugs: {', '.join(sorted(_GRAPH_ATTR_DRUGS))}."
            ),
        )
    if not _GRAPH_ATTR_CSV.exists():
        raise HTTPException(
            status_code=503,
            detail=f"Attribution CSV not found at {_GRAPH_ATTR_CSV}.",
        )

    rows: list[GraphAtomAttribution] = []
    with open(_GRAPH_ATTR_CSV, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row["drug"].strip().lower() == slug:
                rows.append(
                    GraphAtomAttribution(
                        atom_index=int(row["atom_index"]),
                        atom_symbol=row["atom_symbol"].strip(),
                        importance_score=float(row["importance_score"]),
                        rank=int(row["rank"]),
                    )
                )

    rows.sort(key=lambda r: r.importance_score, reverse=True)
    return GraphAttributionResponse(drug=slug, atom_count=len(rows), attributions=rows)
